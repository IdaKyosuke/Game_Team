...
public class StatusUI : MonoBehaviour
{
    ...
    private void SetupText()
    {
        ...
        m_sliderValue[0].text = m_playerStatus.Current.hp.ToString() + " / " + m_playerStatus.Total.hp.ToString();

        m_slider[1].maxValue = m_playerStatus.Total.mp;
        m_slider[1].value = m_playerStatus.Current.mp;
        m_sliderValue[1].text = m_playerStatus.Current.mp.ToString() + " / " + m_playerStatus.Total.mp.ToString();
        ...
        m_statusText[6].text = m_playerStatus.Level == m_playerStatus.MaxLevel ? "最大レベル" :
            (m_playerStatus.Total.requiredExp - m_playerStatus.Current.requiredExp).ToString();
    }
}